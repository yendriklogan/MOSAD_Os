...
MOSAD_O::MOSAD_O() : Algorithm(NULL) {

}

void MOSAD_O::initialize() {

}

void MOSAD_O::initialize(Requirements* config) {
    ...
    while (Ti >= Tf && FE <= MFE) {
        ...
        for (int i = 0; i < N; i++)//for each sub-problem
        {
            ...
            for (int j = 0; j < L; j++)
            {
                ...
                Interval v2 = g(current, unitVectors[i], Z_min);
                Interval v3 = g(P->get(i), unitVectors[i], Z_min);


                om->outranking(newSolution.getObjectives(), P->get(i).getObjectives());
                bool xPy = om->getStrictPreference_XY();
                om->outranking(newSolution.getObjectives(), current.getObjectives());
                bool xPz = om->getStrictPreference_XY();
...
Solution MOSAD_O::perturbation(Solution original)
{
    ...
    randomSolutions->add(P->get(n1));
    randomSolutions->add(P->get(n2));
    randomSolutions->add(P->get(n3));
...
double MOSAD_O::probabilityFunction(Solution nueva, Solution actual, double T)
{
...
Interval MOSAD_O::g(Solution original, Interval* vector, Interval* z_min)
{
    Interval c = 0;
    //int t = this->problem_->getNumberOfObjectives();
    objs = original.getObjectives();
    //Interval* abs = (Interval*)Constantes::generateVector(t, Constantes::INTERVAL);
...
void MOSAD_O::obtainIdealPoint(Solution p)
{
    for (int i = 0; i < this->problem_->getNumberOfObjectives(); ++i) {
